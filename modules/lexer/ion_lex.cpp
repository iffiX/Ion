...
namespace Ion {
    IonLexer::IonLexer() {
        ...
        _lexer_st.indent_stack.push(0);
        _logger = spdlog::stdout_color_st("Ion");
        _logger->set_pattern("[Lexer] %v");
    }

    IonLexer::IonLexer(const std::string filename) {
        yylex_init(&_lexer);
        yyset_extra(&_lexer_st, _lexer);
        yyset_debug(1, _lexer);
        _lexer_st.major_offset = 0;
        _lexer_st.minor_offset = 0;
        _lexer_st.indent_stack.push(0);
        _logger = spdlog::stdout_color_st("Ion");
    ...
    bool IonLexer::openFile(const std::string filename) {
        ...
        _lexer_st.major_offset = 0;
        _lexer_st.minor_offset = 0;
        while (!_lexer_st.indent_stack.empty())
            _lexer_st.indent_stack.pop();
        _lexer_st.indent_stack.push(0);
